...
func auditAttrs(ctx Context, r *http.Request, action string, attrs ...slog.Attr) []any {
...
func AuditSuccess(ctx Context, r *http.Request, action string, attrs ...slog.Attr) {
	args := append(auditAttrs(ctx, r, action, attrs...), slog.Bool("success", true))
	ctx.Logger().Info("audit", args...)
}

func AuditFailure(ctx Context, r *http.Request, action string, err error, attrs ...slog.Attr) {
