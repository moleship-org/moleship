	...
	r.Group(func(r chi.Router) {
		r.Route("/systemd", func(r chi.Router) {
			...
			r.Route("/units/{unit}", func(r chi.Router) {
				...
				r.Post("/enable", h.EnableUnit)
				r.Post("/disable", h.DisableUnit)
			})
		})
	})
}
...
func (h *Systemd) EnableUnit(w http.ResponseWriter, r *http.Request) {
	ctx := apiutil.From(w, r)
	unit := chi.URLParam(r, "unit")

	if err := h.sys.EnableUnit(r.Context(), unit); err != nil {
		apiutil.AuditFailure(ctx, r, "systemd.unit.enable", err, slog.String("unit", unit))
		writeSystemdError(ctx, err)
		return
	}

	apiutil.AuditSuccess(ctx, r, "systemd.unit.enable", slog.String("unit", unit))
	ctx.Status(http.StatusNoContent)
}

func (h *Systemd) DisableUnit(w http.ResponseWriter, r *http.Request) {
	ctx := apiutil.From(w, r)
	unit := chi.URLParam(r, "unit")

	if err := h.sys.DisableUnit(r.Context(), unit); err != nil {
		apiutil.AuditFailure(ctx, r, "systemd.unit.disable", err, slog.String("unit", unit))
		writeSystemdError(ctx, err)
		return
	}

	apiutil.AuditSuccess(ctx, r, "systemd.unit.disable", slog.String("unit", unit))
	ctx.Status(http.StatusNoContent)
}
...
func writeSystemdError(ctx apiutil.Context, err error) {
	apiutil.WriteMappedError(ctx, "Systemd endpoint error", err, "unexpected error calling systemd",
		...
		apiutil.ErrorMapping{Target: systemd.ErrUnitTransientOrGenerated, Status: http.StatusBadRequest, Message: "unit is generated by a systemd generator (e.g. Podman Quadlet) and cannot be enabled/disabled via systemctl; configure the [Install] section (WantedBy=/RequiredBy=/Alias=) in its source definition instead"},
